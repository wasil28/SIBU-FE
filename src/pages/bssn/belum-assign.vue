...
const pageTitle = 'Belum Assign TTE'
...
const fetchData = async (isAllData = false) => { // Penambahan param isAllData untuk akomodir export seluruh data (tanpa limit)
  ...
  const result = await AssignPenugasanApi.getAllDataBelumAssign({
    start: page.value,
        ...
        >
          <!-- <template #aksi-data="{ row }">
            ...
            <template #namaDokumen-data="{ row }">
                ...
                <ul>
                  <li v-if="row.flag_ijazah == 1"> - Ijazah</li>
                  <li v-if="row.flag_ijazah_akta == 1"> - Ijazah Akta</li>
                  <li v-if="row.flag_transkrip == 1"> - Transkrip</li>
                </ul>
            </template>
            ...
            <template #tanggalDibuat-data="{ row }">
                <!-- {{ `${row.createdAt}` }} -->
                {{ `${moment.utc(row.user_date_create).tz('Asia/Jakarta').format('DD/MM/YYYY HH:mm')}` }}
            </template>
